...
export async function sendOTP({ emailAddress, otp }) {
  try {

    const sendOTPEmail = await resend.emails.send({
      from: process.env.RESEND_SENDER_EMAIL || "Your Booking Team <no-reply@yourdomain.com>",
      to: emailAddress,
      subject: "LLM GEM - Verification Code",
      html: `
        <div style="font-family: Arial, sans-serif; padding: 20px; max-width: 600px; margin: 0 auto;">
          <h2 style="color: #333;">Verification Code</h2>
          <p>Your verification code is: <strong>${otp}</strong></p>
          <p style="margin-top: 20px; font-size: 14px; color: #666;">
            This code will be used to verify your email address. Please keep it safe and secure.
          </p>
        </div>
      `,
    })
