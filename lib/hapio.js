...
export async function getAvailability(from, to, locationId) {
  try {
    ...
    const formatForHapio = (dateString) => {
      return formatISO(new Date(dateString), { representation: 'complete' })
        .replace(/\.\d{3}Z$/, ''); // No Z, as specified
    };

    url.searchParams.append('from', formatForHapio(from));
    url.searchParams.append('to', formatForHapio(to));
